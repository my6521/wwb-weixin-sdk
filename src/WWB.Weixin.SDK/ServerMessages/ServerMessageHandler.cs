...
using WWB.Weixin.SDK.ServerMessages.Request.Events;
...
namespace WWB.Weixin.SDK.ServerMessages
{
    public class ServerMessageHandler
    {
            ...
            where T : class, IFromMessage
        {
            ToMessageBase toMessage = null;
            var type = typeof(T);
            var handler = serviceProvider.GetService<IWxSdkEventsHandler>();
            if (handler == null)
            {
                throw new WxSdkException($"接收公众号服务端事件消息需要先注册 {nameof(IWxSdkEventsHandler)} ！");
            }

            IFromMessage fromMessage = XmlUtility.DeserializeObject<T>(xmlStr);
            if (fromMessage != null)
